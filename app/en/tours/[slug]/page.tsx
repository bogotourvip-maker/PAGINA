...
export async function generateMetadata({ params }: TourPageProps): Promise<Metadata> {
  ...
  const content = getTourContentEn(slug)

  if (!tour || !content) {
    return { title: "Tour not found" }
  }

  const url = `https://bogotourvip.com/en/tours/${tour.slug}`
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      locale: "en_US",
      images: [{ url: tour.heroImage, width: 1200, height: 630, alt: t.name }],
    },
    twitter: {
...
export default async function TourPageEn({ params }: TourPageProps) {
  const { slug } = await params
  const tour = getTourBySlug(slug)
  const content = getTourContentEn(slug)
  ...
  const t = getLocalizedTour(tour, "en")
  const whatsappLink = localizedWhatsappLink(t.name, "en")
  const relatedTours = tours.filter((r) => r.slug !== tour.slug).slice(0, 3)
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 pb-8 sm:pb-12">
            <div className="inline-flex items-center gap-2 mb-4">
              <span className="px-3 py-1 bg-[#d4af37] text-black text-xs font-semibold rounded-full">{t.category}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-3 gap-8 lg:gap-12">
            <div className="lg:col-span-2">
              ...
              <div className="mb-10">
                <h2 className="text-2xl font-bold text-white mb-5">What you&apos;ll visit</h2>
              ...
              {tour.gallery.length > 0 && (
                <div className="mb-10">
                  <h2 className="text-2xl font-bold text-white mb-5">Gallery</h2>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {tour.gallery.map((image, index) => (
                      <div key={index} className="relative aspect-[4/3] rounded-xl overflow-hidden">
                        <Image
                          src={image || "/placeholder.svg"}
                          alt={`${t.name} - photo ${index + 1}`}
                          fill
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 bg-white/5 border border-white/10 rounded-2xl p-6">
                <p className="text-sm text-white/50 mb-1">From</p>
                <p className="text-2xl font-bold text-white mb-5">
                  {tour.priceFrom === "Consultar" ? "On request" : tour.priceFrom}
                </p>

                <div className="mb-6">
                  <p className="text-xs uppercase tracking-wider text-white/50 mb-3">Includes</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {relatedTours.map((related) => {
              const rt = getLocalizedTour(related, "en")
              return (
                <Link
                  key={related.slug}
                  href={`/en/tours/${related.slug}`}
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-white/50 mt-1 line-clamp-2">{rt.shortDescription}</p>
                  </div>
                </Link>
              )
            })}
